...
int main(){
	...
	// -- Section 1: Current player items
		...
		if (scoreFor(0, &currentState) == scoreFor(0, &originalState)){
			printf("Test 1.a success: scores remain the same for player 0\n");
		} else {
			printf("Test 1.a failure: scrores should have remained the same for player 0\n");
		}
	...
	// -- Section 2: Other players things should remain the same
		// -- 2.a: Number of cards stay the same in both hand and deck
		// -- 2.b: Exact cards should really stay the same everywhere
		// -- 2.c: Score should remain the same
